...
def sell(coins, account):
    ...
    while i < len(coins):
        if coins[i].sell["type"] == "market":
                coins[i].orders, account = coins[i].cancel_orders("sell", account)
                quantity = account.coins[coins[i].market_name]["available"]
                if quantity != 0:
...
def buy(coins, account):
    ...
    for index in to_buy_indexes:
        if coins[index].buy["type"] == "market":
            account = coins[index].market_buy(account, euro_split + ret)
        elif coins[index].buy["type"] == "limit":
            account = coins[index].limit_buy(account, euro_split + ret)
    return coins
